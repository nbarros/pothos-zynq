...
int test(const int index)
{
  ...
  while(run_ctrl) {
    ...
    //printf("iter %u recv %zu bytes into handle %d (time %lf us) [TimeStamp from GPIO : %u] State %x\n",counter,len,handle,cur_tdiff,Xil_In32((uint32_t)mmapaddr + GPIO_CH0_OFFSET),Xil_In32((uint32_t)mmapaddr + GPIO_CH1_OFFSET));
